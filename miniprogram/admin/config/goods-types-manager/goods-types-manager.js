...
Page({
  ...
  onUnload: async function () {
    ...
    // var ret = query.select("#title-0").fields({
    //   context: true,
    //   id : true,
    //   properties : ["value"]
    // }).exec(function (res){
    ...
    for (var index = 0; index < this.data.goodsTypes.length; index++) {
      ...
      if (item.old_title == item.title
        && item.old_desc == item.desc) {
        continue//忽略虽然有更改过程，但结果与最初结果一样的
      }
      var type = {
        title: item.title,
        desc: item.desc,
      ...
      if (!item.img.startsWith("cloud://") || item.old_img != null) {
        ...
        if (item.old_img != null) {
          ret = await wx.cloud.deleteFile({
            fileList: [item.old_img]
          })
